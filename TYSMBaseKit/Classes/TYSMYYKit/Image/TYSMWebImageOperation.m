...
#define TYSM_MIN_PROGRESSIVE_TIME_INTERVAL 0.2
#define TYSM_MIN_PROGRESSIVE_BLUR_TIME_INTERVAL 0.4

/// Returns YES if the right-bottom pixel is filled.
static BOOL TYSMCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
static NSData *TYSM_JPEGSOSMarker() {
...
static NSMutableSet *TYSM_URLBlacklist;
static dispatch_semaphore_t TYSM_URLBlacklistLock;

static void TYSM_URLBlacklistInit() {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        TYSM_URLBlacklist = [NSMutableSet new];
        TYSM_URLBlacklistLock = dispatch_semaphore_create(1);
    });
}

static BOOL TYSM_URLBlacklistContains(NSURL *url) {
    if (!url || url == (id)[NSNull null]) return NO;
    TYSM_URLBlacklistInit();
    dispatch_semaphore_wait(TYSM_URLBlacklistLock, DISPATCH_TIME_FOREVER);
    BOOL contains = [TYSM_URLBlacklist containsObject:url];
    dispatch_semaphore_signal(TYSM_URLBlacklistLock);
    return contains;
}

static void TYSM_URLInBlackListAdd(NSURL *url) {
    if (!url || url == (id)[NSNull null]) return;
    TYSM_URLBlacklistInit();
    dispatch_semaphore_wait(TYSM_URLBlacklistLock, DISPATCH_TIME_FOREVER);
    [TYSM_URLBlacklist addObject:url];
    dispatch_semaphore_signal(TYSM_URLBlacklistLock);
}


@interface TYSMWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) TYSMImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) TYSMWebImageProgressBlock progress;
@property (nonatomic, copy) TYSMWebImageTransformBlock transform;
@property (nonatomic, copy) TYSMWebImageCompletionBlock completion;
@end


@implementation TYSMWebImageOperation
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & TYSMWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, TYSMWebImageFromNone, TYSMWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & TYSMWebImageOptionUseNSURLCache) &&
            !(_options & TYSMWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:TYSMImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, TYSMWebImageFromMemoryCache, TYSMWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & TYSMWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:TYSMImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:TYSMImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & TYSMWebImageOptionIgnoreFailedURL) && TYSM_URLBlacklistContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, TYSMWebImageFromNone, TYSMWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, TYSMWebImageFromNone, TYSMWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, TYSMWebImageFromDiskCache, TYSMWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & TYSMWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([TYSMWebImageOperation _imageQueue], ^{
                        TYSMImageCacheType cacheType = (_options & TYSMWebImageOptionIgnoreDiskCache) ? TYSMImageCacheTypeMemory : TYSMImageCacheTypeAll;
            ...
            if (!image) {
                ...
                if (_options & TYSMWebImageOptionIgnoreFailedURL) {
                    if (TYSM_URLBlacklistContains(_request.URL)) {
                        ...
                        TYSM_URLInBlackListAdd(_request.URL);
                    }
                }
            }
            if (_completion) _completion(image, _request.URL, TYSMWebImageFromRemote, TYSMWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & TYSMWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & TYSMWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & TYSMWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & TYSMWebImageOptionProgressiveBlur) > 0;
        ...
        NSTimeInterval min = progressiveBlur ? TYSM_MIN_PROGRESSIVE_BLUR_TIME_INTERVAL : TYSM_MIN_PROGRESSIVE_TIME_INTERVAL;
        NSTimeInterval now = CACurrentMediaTime();
        if (now - _lastProgressiveDecodeTimestamp < min) return;
        
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[TYSMImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == TYSMImageTypeUnknown ||
            _progressiveDecoder.type == TYSMImageTypeWebP ||
            _progressiveDecoder.type == TYSMImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != TYSMImageTypeJPEG &&
                _progressiveDecoder.type != TYSMImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            TYSMImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, TYSMWebImageFromRemote, TYSMWebImageStageProgress, nil);
            ...
            if (_progressiveDecoder.type == TYSMImageTypeJPEG) {
                ...
                NSRange markerRange = [_data rangeOfData:TYSM_JPEGSOSMarker() options:kNilOptions range:scanRange];
                _progressiveScanedLength = _data.length;
                if (markerRange.location == NSNotFound) return;
                if ([self isCancelled]) return;
                
            } else if (_progressiveDecoder.type == TYSMImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!TYSMCGImageLastPixelFilled(image.CGImage)) return;
            ...
            image = [image tysm_imageByBlurRadius:radius tintColor:nil tintMode:0 saturation:1 maskImage:nil];
            
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, TYSMWebImageFromRemote, TYSMWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & TYSMWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & TYSMWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[TYSMImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image tysm_imageByDecoded];
                    if ([((TYSMImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    TYSMImageDecoder *decoder = [TYSMImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                TYSMImageType imageType = TYSMImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case TYSMImageTypeJPEG:
                    case TYSMImageTypeGIF:
                    case TYSMImageTypePNG:
                    case TYSMImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == TYSMImageTypeGIF ||
                                imageType == TYSMImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & TYSMWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, TYSMWebImageFromNone, TYSMWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & TYSMWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & TYSMWebImageOptionAllowBackgroundTask) && ![UIApplication isAppExtension]) {
