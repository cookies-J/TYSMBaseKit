...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, TYSMImageType) {
    TYSMImageTypeUnknown = 0, ///< unknown
    TYSMImageTypeJPEG,        ///< jpeg, jpg
    TYSMImageTypeJPEG2000,    ///< jp2
    TYSMImageTypeTIFF,        ///< tiff, tif
    TYSMImageTypeBMP,         ///< bmp
    TYSMImageTypeICO,         ///< ico
    TYSMImageTypeICNS,        ///< icns
    TYSMImageTypeGIF,         ///< gif
    TYSMImageTypePNG,         ///< png
    TYSMImageTypeWebP,        ///< webp
    TYSMImageTypeOther,       ///< other image format
};
 ...
 */
@interface TYSMImageFrame : NSObject <NSCopying>
 ...
 */
@interface TYSMImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) TYSMImageType type;          ///< Image data type.
 ...
 */
- (nullable TYSMImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in TYSMImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    YYImageEncoder *jpegEncoder = [[YYImageEncoder alloc] initWithType:TYSMImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    YYImageEncoder *gifEncoder = [[YYImageEncoder alloc] initWithType:TYSMImageTypeGIF];
 ...
 */
@interface TYSMImageEncoder : NSObject

@property (nonatomic, readonly) TYSMImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(TYSMImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(TYSMImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(TYSMImageDecoder *)decoder type:(TYSMImageType)type quality:(CGFloat)quality;
...
@interface UIImage (TYSMImageCoder)
 ...
 */
@property (nonatomic) BOOL tysm_isDecodedForDisplay;
 ...
 */
- (void)tysm_saveToAlbumWithCompletionBlock:(nullable void(^)(NSURL * _Nullable assetURL, NSError * _Nullable error))completionBlock;
 ...
 */
- (nullable NSData *)tysm_imageDataRepresentation;
...
CG_EXTERN TYSMImageType TYSMImageDetectType(CFDataRef data);

/// Convert TYSMImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable TYSMImageTypeToUTType(TYSMImageType type);

/// Convert UTI (such as kUTTypeJPEG) to TYSMImageType.
CG_EXTERN TYSMImageType TYSMImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable TYSMImageTypeGetExtension(TYSMImageType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef TYSMCGColorSpaceGetDeviceRGB();

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef TYSMCGColorSpaceGetDeviceGray();

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL TYSMCGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL TYSMCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation TYSMUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger TYSMUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable TYSMCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable TYSMCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable TYSMCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable TYSMCGImageCreateEncodedData(CGImageRef imageRef, TYSMImageType type, CGFloat quality);


/**
 Whether WebP is available in YYImage.
 */
CG_EXTERN BOOL TYSMImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger TYSMImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable TYSMCGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, TYSMImagePreset) {
    TYSMImagePresetDefault = 0,  ///< default preset.
    TYSMImagePresetPicture,      ///< digital picture, like portrait, inner shot
    TYSMImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    TYSMImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    TYSMImagePresetIcon,         ///< small-sized colorful images
    TYSMImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is TYSMImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable TYSMCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             TYSMImagePreset preset);
