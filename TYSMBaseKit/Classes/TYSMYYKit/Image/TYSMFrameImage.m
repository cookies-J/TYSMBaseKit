...
- (instancetype)initWithImagePaths:(NSArray *)paths frameDurations:(NSArray *)frameDurations loopCount:(NSUInteger)loopCount {
    ...
    CGFloat scale = firstPath.tysm_pathScale;
    UIImage *firstCG = [[[UIImage alloc] initWithData:firstData] tysm_imageByDecoded];
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    if (_imagePaths) {
        ...
        CGFloat scale = [path tysm_pathScale];
        NSData *data = [NSData dataWithContentsOfFile:path];
        return [[UIImage imageWithData:data scale:scale] tysm_imageByDecoded];
    } else if (_imageDatas) {
        if (index >= _imageDatas.count) return nil;
        NSData *data = _imageDatas[index];
        return [[UIImage imageWithData:data scale:[UIScreen mainScreen].scale] tysm_imageByDecoded];
    } else {
