...
/// Image cache type
typedef NS_OPTIONS(NSUInteger, TYSMImageCacheType) {
    /// No value.
    TYSMImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    TYSMImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    TYSMImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    TYSMImageCacheTypeAll    = TYSMImageCacheTypeMemory | TYSMImageCacheTypeDisk,
};
 ...
 */
@interface TYSMImageCache : NSObject
...
/**
 ...
 @discussion If the `type` contain `TYSMImageCacheTypeMemory`, then the `image` will
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `TYSMImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(TYSMImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `TYSMImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(TYSMImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(TYSMImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, TYSMImageCacheType type))block;
