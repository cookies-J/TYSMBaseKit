...
typedef NS_ENUM(NSUInteger, TYSMWebImageFromType) {
    ...
    TYSMWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    TYSMWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    TYSMWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    TYSMWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, TYSMWebImageStage) {
    
    /// Incomplete, progressive image.
    TYSMWebImageStageProgress  = -1,
    
    /// Cancelled.
    TYSMWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    TYSMWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^TYSMWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);
 ...
 */
typedef UIImage * _Nullable (^TYSMWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^TYSMWebImageCompletionBlock)(UIImage * _Nullable image,
                                          NSURL *url,
                                          TYSMWebImageFromType from,
                                          TYSMWebImageStage stage,
 ...
 */
@interface TYSMWebImageManager : NSObject
 ...
 */
- (instancetype)initWithCache:(nullable TYSMImageCache *)cache
 ...
 */
- (nullable TYSMWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) TYSMImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) TYSMWebImageTransformBlock sharedTransformBlock;
