...
+ (TYSMImage *)imageNamed:(NSString *)name {
    ...
    for (int s = 0; s < scales.count; s++) {
        ...
        NSString *scaledName = [res tysm_stringByAppendingNameScale:scale];
...
+ (TYSMImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (TYSMImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (TYSMImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}

- (instancetype)initWithContentsOfFile:(NSString *)path {
    NSData *data = [NSData dataWithContentsOfFile:path];
    return [self initWithData:data scale:path.tysm_pathScale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        TYSMImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.tysm_isDecodedForDisplay = YES;
    }
    return self;
}
