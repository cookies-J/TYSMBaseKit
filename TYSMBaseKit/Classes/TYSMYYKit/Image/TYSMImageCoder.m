...
#import "webp/mux.h"
#else
#define TYSMIMAGE_WEBP_ENABLED 0
#endif
#endif
...
#define TYSM_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define TYSM_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

static inline uint16_t TYSM_swap_endian_uint16(uint16_t value) {
    return
    (uint16_t) ((value & 0x00FF) << 8) |
    (uint16_t) ((value & 0xFF00) >> 8) ;
}

static inline uint32_t TYSM_swap_endian_uint32(uint32_t value) {
    return
...
typedef enum {
    TYSM_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    TYSM_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    TYSM_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} TYSM_PNG_ALPHA_TYPE;

typedef enum {
    TYSM_PNG_DISPOSE_OP_NONE = 0,
    TYSM_PNG_DISPOSE_OP_BACKGROUND = 1,
    TYSM_PNG_DISPOSE_OP_PREVIOUS = 2,
} TYSM_png_dispose_op;

typedef enum {
    TYSM_PNG_BLEND_OP_SOURCE = 0,
    TYSM_PNG_BLEND_OP_OVER = 1,
} TYSM_png_blend_op;
...
typedef struct {
    ...
    uint8_t color_type;         ///< see TYSM_PNG_ALPHA_TYPE
...
typedef struct {
    ...
    uint8_t dispose_op;        ///< see TYSM_png_dispose_op
    uint8_t blend_op;          ///< see TYSM_png_blend_op
} TYSM_png_chunk_fcTL;
...
typedef struct {
    ...
    TYSM_png_chunk_fcTL frame_control;
} TYSM_png_frame_info;

typedef struct {
    TYSM_png_chunk_IHDR header;   ///< png header
    TYSM_png_chunk_info *chunks;      ///< chunks
    uint32_t chunk_num;          ///< count of chunks
    
    TYSM_png_frame_info *apng_frames; ///< frame info, NULL if not apng
...
static void TYSM_png_chunk_IHDR_read(TYSM_png_chunk_IHDR *IHDR, const uint8_t *data) {
    IHDR->width = TYSM_swap_endian_uint32(*((uint32_t *)(data)));
    IHDR->height = TYSM_swap_endian_uint32(*((uint32_t *)(data + 4)));
...
static void TYSM_png_chunk_IHDR_write(TYSM_png_chunk_IHDR *IHDR, uint8_t *data) {
    *((uint32_t *)(data)) = TYSM_swap_endian_uint32(IHDR->width);
    *((uint32_t *)(data + 4)) = TYSM_swap_endian_uint32(IHDR->height);
...
static void TYSM_png_chunk_fcTL_read(TYSM_png_chunk_fcTL *fcTL, const uint8_t *data) {
    fcTL->sequence_number = TYSM_swap_endian_uint32(*((uint32_t *)(data)));
    fcTL->width = TYSM_swap_endian_uint32(*((uint32_t *)(data + 4)));
    fcTL->height = TYSM_swap_endian_uint32(*((uint32_t *)(data + 8)));
    fcTL->x_offset = TYSM_swap_endian_uint32(*((uint32_t *)(data + 12)));
    fcTL->y_offset = TYSM_swap_endian_uint32(*((uint32_t *)(data + 16)));
    fcTL->delay_num = TYSM_swap_endian_uint16(*((uint16_t *)(data + 20)));
    fcTL->delay_den = TYSM_swap_endian_uint16(*((uint16_t *)(data + 22)));
    fcTL->dispose_op = data[24];
    fcTL->blend_op = data[25];
}

static void TYSM_png_chunk_fcTL_write(TYSM_png_chunk_fcTL *fcTL, uint8_t *data) {
    *((uint32_t *)(data)) = TYSM_swap_endian_uint32(fcTL->sequence_number);
    *((uint32_t *)(data + 4)) = TYSM_swap_endian_uint32(fcTL->width);
    *((uint32_t *)(data + 8)) = TYSM_swap_endian_uint32(fcTL->height);
    *((uint32_t *)(data + 12)) = TYSM_swap_endian_uint32(fcTL->x_offset);
    *((uint32_t *)(data + 16)) = TYSM_swap_endian_uint32(fcTL->y_offset);
    *((uint16_t *)(data + 20)) = TYSM_swap_endian_uint16(fcTL->delay_num);
    *((uint16_t *)(data + 22)) = TYSM_swap_endian_uint16(fcTL->delay_den);
    data[24] = fcTL->dispose_op;
    data[25] = fcTL->blend_op;
}

// convert double value to fraction
static void TYSM_png_delay_to_fraction(double duration, uint16_t *num, uint16_t *den) {
    if (duration >= 0xFF) {
        *num = 0xFF;
        *den = 1;
...
static double TYSM_png_delay_to_seconds(uint16_t num, uint16_t den) {
    if (den == 0) {
        return num / 100.0;
    } else {
...
static bool TYSM_png_validate_animation_chunk_order(TYSM_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != TYSM_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != TYSM_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        TYSM_png_chunk_info *chunk = chunks + i;
        switch (chunk->fourcc) {
            case TYSM_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case TYSM_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case TYSM_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case TYSM_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != TYSM_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case TYSM_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != TYSM_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != TYSM_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static void TYSM_png_info_release(TYSM_png_info *info) {
    if (info) {
...
/**
 ...
 @return A png info object, you may call TYSM_png_info_release() to release it.
 Returns NULL if an error occurs.
 */
static TYSM_png_info *TYSM_png_info_create(const uint8_t *data, uint32_t length) {
    if (length < 32) return NULL;
    if (*((uint32_t *)data) != TYSM_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != TYSM_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    
    uint32_t chunk_realloc_num = 16;
    TYSM_png_chunk_info *chunks = malloc(sizeof(TYSM_png_chunk_info) * chunk_realloc_num);
    ...
    do {
        if (chunk_num >= chunk_capacity) {
            TYSM_png_chunk_info *new_chunks = realloc(chunks, sizeof(TYSM_png_chunk_info) * (chunk_capacity + chunk_realloc_num));
            if (!new_chunks) {
                free(chunks);
                return NULL;
            }
        ...
        TYSM_png_chunk_info *chunk = chunks + chunk_num;
        const uint8_t *chunk_data = data + offset;
        chunk->offset = offset;
        chunk->length = TYSM_swap_endian_uint32(*((uint32_t *)chunk_data));
        ...
        chunk->crc32 = TYSM_swap_endian_uint32(*((uint32_t *)(chunk_data + 8 + chunk->length)));
        chunk_num++;
        offset += 12 + chunk->length;
        
        switch (chunk->fourcc) {
            case TYSM_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = TYSM_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
                    apng_loop_num = TYSM_swap_endian_uint32(*((uint32_t *)(chunk_data + 12)));
                } else {
                    apng_chunk_error = true;
                }
            } break;
            case TYSM_FOUR_CC('f', 'c', 'T', 'L') :
            case TYSM_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == TYSM_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
                ...
                if (chunk->length > 4) {
                    uint32_t sequence = TYSM_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != TYSM_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    // png info
    TYSM_png_info *info = calloc(1, sizeof(TYSM_png_info));
    if (!info) {
        free(chunks);
        return NULL;
    }
    ...
    TYSM_png_chunk_IHDR_read(&info->header, data + chunks->offset + 8);
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!TYSM_png_validate_animation_chunk_order(info->chunks, info->chunk_num, &first_IDAT_index, &first_frame_is_cover)) {
        ...
        info->apng_frames = calloc(apng_frame_number, sizeof(TYSM_png_frame_info));
        if (!info->apng_frames) {
            TYSM_png_info_release(info);
            return NULL;
        }
        ...
        for (int32_t i = 0; i < info->chunk_num; i++) {
            TYSM_png_chunk_info *chunk = info->chunks + i;
            switch (chunk->fourcc) {
                case TYSM_FOUR_CC('I', 'D', 'A', 'T'): {
                    if (info->apng_shared_insert_index == 0) {
                        info->apng_shared_insert_index = i;
                    }
                    if (first_frame_is_cover) {
                        TYSM_png_frame_info *frame = info->apng_frames + frame_index;
                        frame->chunk_num++;
                ...
                case TYSM_FOUR_CC('f', 'c', 'T', 'L'): {
                    ...
                    TYSM_png_chunk_fcTL_read(&frame->frame_control, data + chunk->offset + 8);
                } break;
                case TYSM_FOUR_CC('f', 'd', 'A', 'T'): {
                    TYSM_png_frame_info *frame = info->apng_frames + frame_index;
                    frame->chunk_num++;
...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *TYSM_png_copy_frame_data_at_index(const uint8_t *data,
                                                const TYSM_png_info *info,
                                                const uint32_t index,
                                                uint32_t *size) {
    if (index >= info->apng_frame_num) return NULL;
    
    TYSM_png_frame_info *frame_info = info->apng_frames + index;
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        TYSM_png_chunk_info *shared_chunk_info = info->chunks + shared_chunk_index;
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                TYSM_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index + c;
                if (insert_chunk_info->fourcc == TYSM_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = TYSM_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = TYSM_FOUR_CC('I', 'D', 'A', 'T');
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = TYSM_swap_endian_uint32(crc);
        ...
        if (shared_chunk_info->fourcc == TYSM_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
            memcpy(tmp, data + shared_chunk_info->offset, 25);
            TYSM_png_chunk_IHDR IHDR = info->header;
            IHDR.width = frame_info->frame_control.width;
            IHDR.height = frame_info->frame_control.height;
            TYSM_png_chunk_IHDR_write(&IHDR, tmp + 8);
            *((uint32_t *)(tmp + 21)) = TYSM_swap_endian_uint32((uint32_t)crc32(0, tmp + 4, 17));
...
CGColorSpaceRef TYSMCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef TYSMCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL TYSMCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, TYSMCGColorSpaceGetDeviceRGB());
}

BOOL TYSMCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, TYSMCGColorSpaceGetDeviceGray());
}
...
static BOOL YYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = TYSMCGColorSpaceGetDeviceRGB();
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef TYSMCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef TYSMCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, TYSMCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, TYSMCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation TYSMUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger TYSMUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef TYSMCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    return TYSMCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

TYSMImageType TYSMImageDetectType(CFDataRef data) {
    if (!data) return TYSMImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return TYSMImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case TYSM_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return TYSMImageTypeTIFF;
        } break;
            
        case TYSM_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return TYSMImageTypeTIFF;
        } break;
            
        case TYSM_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return TYSMImageTypeICO;
        } break;
            
        case TYSM_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return TYSMImageTypeICO;
        } break;
            
        case TYSM_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return TYSMImageTypeICNS;
        } break;
            
        case TYSM_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return TYSMImageTypeGIF;
        } break;
            
        case TYSM_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == TYSM_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return TYSMImageTypePNG;
            }
        } break;
            
        case TYSM_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == TYSM_FOUR_CC('W', 'E', 'B', 'P')) {
                return TYSMImageTypeWebP;
            }
        } break;
        /*
        case TYSM_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return TYSMImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        case TYSM_TWO_CC('B', 'A'):
        case TYSM_TWO_CC('B', 'M'):
        case TYSM_TWO_CC('I', 'C'):
        case TYSM_TWO_CC('P', 'I'):
        case TYSM_TWO_CC('C', 'I'):
        case TYSM_TWO_CC('C', 'P'): { // BMP
            return TYSMImageTypeBMP;
        }
        case TYSM_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return TYSMImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return TYSMImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return TYSMImageTypeJPEG2000;
    
    return TYSMImageTypeUnknown;
}

CFStringRef TYSMImageTypeToUTType(TYSMImageType type) {
    switch (type) {
        case TYSMImageTypeJPEG: return kUTTypeJPEG;
        case TYSMImageTypeJPEG2000: return kUTTypeJPEG2000;
        case TYSMImageTypeTIFF: return kUTTypeTIFF;
        case TYSMImageTypeBMP: return kUTTypeBMP;
        case TYSMImageTypeICO: return kUTTypeICO;
        case TYSMImageTypeICNS: return kUTTypeAppleICNS;
        case TYSMImageTypeGIF: return kUTTypeGIF;
        case TYSMImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

TYSMImageType TYSMImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(TYSMImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(TYSMImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(TYSMImageTypeTIFF),
                (id)kUTTypeBMP : @(TYSMImageTypeBMP),
                (id)kUTTypeICO : @(TYSMImageTypeICO),
                (id)kUTTypeAppleICNS : @(TYSMImageTypeICNS),
                (id)kUTTypeGIF : @(TYSMImageTypeGIF),
                (id)kUTTypePNG : @(TYSMImageTypePNG)};
    });
    if (!uti) return TYSMImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *TYSMImageTypeGetExtension(TYSMImageType type) {
    switch (type) {
        case TYSMImageTypeJPEG: return @"jpg";
        case TYSMImageTypeJPEG2000: return @"jp2";
        case TYSMImageTypeTIFF: return @"tiff";
        case TYSMImageTypeBMP: return @"bmp";
        case TYSMImageTypeICO: return @"ico";
        case TYSMImageTypeICNS: return @"icns";
        case TYSMImageTypeGIF: return @"gif";
        case TYSMImageTypePNG: return @"png";
        case TYSMImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef TYSMCGImageCreateEncodedData(CGImageRef imageRef, TYSMImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == TYSMImageTypeWebP) {
#if TYSMIMAGE_WEBP_ENABLED
        if (quality == 1) {
            return TYSMCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, TYSMImagePresetDefault);
        } else {
            return TYSMCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, TYSMImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = TYSMImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL TYSMImageWebPAvailable() {
    return YES;
}

CFDataRef TYSMCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, TYSMImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > TYSMImagePresetText ? TYSMImagePresetDefault : preset;
...
NSUInteger TYSMImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef TYSMCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, TYSMCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation TYSMImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    TYSMImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    TYSMImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _YYImageDecoderFrame : TYSMImageFrame
...
- (void)dealloc {
    ...
    if (_apngSource) TYSM_png_info_release(_apngSource);
#if TYSMIMAGE_WEBP_ENABLED
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    TYSMImageDecoder *decoder = [[TYSMImageDecoder alloc] initWithScale:scale];
...
- (TYSMImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    TYSMImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    TYSMImageType type = TYSMImageDetectType((__bridge CFDataRef)data);
...
- (TYSMImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _YYImageDecoderFrame *frame = [(_YYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != TYSMImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = TYSMCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
        ...
        CFRelease(imageRef);
        if (!image) return nil;
        image.tysm_isDecodedForDisplay = decoded;
        frame.image = image;
        return frame;
    }
    ...
    image.tysm_isDecodedForDisplay = YES;
    frame.image = image;
    if (extendToCanvas) {
        frame.width = _width;
        frame.height = _height;
...
- (void)_updateSourceAPNG {
    ...
    TYSM_png_info_release(_apngSource);
    _apngSource = nil;
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        TYSM_png_frame_info *fi = apng->apng_frames + i;
        frame.index = i;
        frame.duration = TYSM_png_delay_to_seconds(fi->frame_control.delay_num, fi->frame_control.delay_den);
        frame.hasAlpha = YES;
        ...
        switch (fi->frame_control.dispose_op) {
            case TYSM_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = YYImageDisposeBackground;
            } break;
            case TYSM_PNG_DISPOSE_OP_PREVIOUS: {
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == TYSMImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == TYSMImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = TYSMUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = TYSMCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = TYSM_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, TYSMCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TYSMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
...
@implementation TYSMImageEncoder {
...
- (instancetype)init {
    ...
    return [self initWithType:TYSMImageTypeUnknown];
}

- (instancetype)initWithType:(TYSMImageType)type {
    if (type == TYSMImageTypeUnknown || type >= TYSMImageTypeOther) {
    ...
    switch (type) {
        case TYSMImageTypeJPEG:
        case TYSMImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case TYSMImageTypeTIFF:
        case TYSMImageTypeBMP:
        case TYSMImageTypeGIF:
        case TYSMImageTypeICO:
        case TYSMImageTypeICNS:
        case TYSMImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, TYSMImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, TYSMImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == TYSMImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = TYSMCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return TYSMCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return TYSMCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == TYSMImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = TYSMCGImageCreateEncodedData(decoded, TYSMImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, TYSMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = TYSMCGImageCreateEncodedData(extendedImage, TYSMImageTypePNG, 1);
        if (!frameData) {
    ...
    TYSM_png_info *info = TYSM_png_info_create(firstFrameData.bytes, (uint32_t)firstFrameData.length);
    if (!info) return nil;
    ...
    png_header[0] = TYSM_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = TYSM_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = TYSM_swap_endian_uint32(8); //length
            acTL[1] = TYSM_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            acTL[2] = TYSM_swap_endian_uint32((uint32_t)pngDatas.count); // num frames
            acTL[3] = TYSM_swap_endian_uint32((uint32_t)_loopCount); // num plays
            acTL[4] = TYSM_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); //crc32
            [result appendBytes:acTL length:20];
            
            // insert fcTL (first frame control)
            TYSM_png_chunk_fcTL chunk_fcTL = {0};
            ...
            TYSM_png_delay_to_fraction([(NSNumber *)_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
            chunk_fcTL.delay_num = chunk_fcTL.delay_num;
            chunk_fcTL.delay_den = chunk_fcTL.delay_den;
            chunk_fcTL.dispose_op = TYSM_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = TYSM_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = TYSM_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = TYSM_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
            TYSM_png_chunk_fcTL_write(&chunk_fcTL, fcTL + 8);
            *((uint32_t *)(fcTL + 34)) = TYSM_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
            [result appendBytes:fcTL length:38];
            
            apngSequenceIndex++;
        }
        
        if (!insertAfter && insertBefore && chunk->fourcc != TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
            // insert fcTL and fdAT (APNG frame control and data)
            
            for (int i = 1; i < pngDatas.count; i++) {
                NSData *frameData = pngDatas[i];
                TYSM_png_info *frame = TYSM_png_info_create(frameData.bytes, (uint32_t)frameData.length);
                if (!frame) {
                    TYSM_png_info_release(info);
                    return nil;
                }
                ...
                TYSM_png_delay_to_fraction([(NSNumber *)_durations[i] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
                chunk_fcTL.delay_num = chunk_fcTL.delay_num;
                chunk_fcTL.delay_den = chunk_fcTL.delay_den;
                chunk_fcTL.dispose_op = TYSM_PNG_DISPOSE_OP_BACKGROUND;
                chunk_fcTL.blend_op = TYSM_PNG_BLEND_OP_SOURCE;
                ...
                *((uint32_t *)(fcTL + 34)) = TYSM_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    TYSM_png_chunk_info *dchunk = frame->chunks + d;
                    if (dchunk->fourcc == TYSM_FOUR_CC('I', 'D', 'A', 'T')) {
                        uint32_t length = TYSM_swap_endian_uint32(dchunk->length + 4);
                        [result appendBytes:&length length:4]; //length
                        uint32_t fourcc = TYSM_FOUR_CC('f', 'd', 'A', 'T');
                        [result appendBytes:&fourcc length:4]; //fourcc
                        uint32_t sq = TYSM_swap_endian_uint32(apngSequenceIndex);
                        ...
                        uint32_t crc = TYSM_swap_endian_uint32((uint32_t)crc32(0, bytes, dchunk->length + 8));
                        [result appendBytes:&crc length:4]; //crc
                        
                        apngSequenceIndex++;
                    }
                }
                TYSM_png_info_release(frame);
            }
        }
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = TYSMCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, TYSMImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == TYSMImageTypePNG) return [self _encodeAPNG];
    if (_type == TYSMImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(TYSMImageType)type quality:(CGFloat)quality {
    TYSMImageEncoder *encoder = [[TYSMImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(TYSMImageDecoder *)decoder type:(TYSMImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (TYSMImageCoder)

- (instancetype)tysm_imageByDecoded {
    if (self.tysm_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = TYSMCGImageCreateDecodedCopy(imageRef, YES);
    ...
    newImage.tysm_isDecodedForDisplay = YES;
    return newImage;
}

- (BOOL)tysm_isDecodedForDisplay {
    if (self.images.count > 1) return YES;
    NSNumber *num = objc_getAssociatedObject(self, @selector(tysm_isDecodedForDisplay));
    return [num boolValue];
}

- (void)setTysm_isDecodedForDisplay:(BOOL)isDecodedForDisplay {
    objc_setAssociatedObject(self, @selector(tysm_isDecodedForDisplay), @(isDecodedForDisplay), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)tysm_saveToAlbumWithCompletionBlock:(void(^)(NSURL *assetURL, NSError *error))completionBlock {
...
- (NSData *)tysm_imageDataRepresentation {
...
- (NSData *)_imageDataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[TYSMImage class]]) {
        TYSMImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == TYSMImageTypeGIF ||
                    image.animatedImageType == TYSMImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = TYSMCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
