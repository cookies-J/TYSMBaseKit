...
@interface TYSM_AnimatedImageView() {
    @package
    UIImage <TYSMAnimatedImage> *_curAnimatedImage;
...
@property (nonatomic, weak) TYSM_AnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <TYSMAnimatedImage> *curImage;
@end

@implementation _YYAnimatedImageViewFetchOperation
- (void)main {
    __strong TYSM_AnimatedImageView *view = _view;
    if (!view) return;
...
@implementation TYSM_AnimatedImageView
...
- (void)imageChanged {
    ...
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(TYSMAnimatedImage)]) {
        newImageFrameCount = ((UIImage<TYSMAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<TYSMAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<TYSMAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)calcMaxBufferCount {
    ...
    int64_t total = [UIDevice currentDevice].tysm_memoryTotal;
    int64_t free = [UIDevice currentDevice].tysm_memoryFree;
...
- (void)step:(CADisplayLink *)link {
    UIImage <TYSMAnimatedImage> *image = _curAnimatedImage;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(TYSMAnimatedImage)];
    multi = (ani && ((UIImage <TYSMAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"YYAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(TYSMAnimatedImage)];
    multi = (ani && ((UIImage <TYSMAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
