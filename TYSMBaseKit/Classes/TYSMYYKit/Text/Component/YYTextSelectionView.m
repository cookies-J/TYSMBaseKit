...
- (void)setDotDirection:(TYSMTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == TYSMTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == TYSMTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == TYSMTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == TYSMTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == TYSMTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == TYSMTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == TYSMTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == TYSMTextDirectionLeft) {
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber.dotDirection = TYSMTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [YYSelectionGrabber new];
    _endGrabber.dotDirection = TYSMTextDirectionBottom;
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? TYSMTextDirectionRight : TYSMTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? TYSMTextDirectionLeft : TYSMTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = tysm_CGRectPixelRound(caretRect);
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = tysm_CGPointGetDistanceToPoint(point, tysm_CGRectGetCenter(startRect));
        CGFloat distEnd = tysm_CGPointGetDistanceToPoint(point, tysm_CGRectGetCenter(endRect));
