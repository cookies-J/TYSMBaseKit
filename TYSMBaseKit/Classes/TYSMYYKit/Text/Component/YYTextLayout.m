...
@implementation TYSMTextLinePositionSimpleModifier
...
- (id)copyWithZone:(NSZone *)zone {
    TYSMTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation YYTextContainer {
    ...
    TYSMTextTruncationType _truncationType;
...
- (TYSMTextTruncationType)truncationType {
    Getter(TYSMTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(TYSMTextTruncationType)truncationType {
...
+ (YYTextLayout *)layoutWithContainer:(YYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ...
    if (needFixJoinedEmojiBug) {
        [((NSMutableAttributedString *)text) tysm_setClearColorToJoinedEmoji];
    }
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect, tysm_UIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    if (needTruncation) {
        ...
        if (container.truncationType != TYSMTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                if (runCount > 0) {
                    ...
                    [attrs removeObjectsForKeys:[NSMutableAttributedString tysm_allDiscontinuousAttributeKeys]];
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:TYSMTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == TYSMTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == TYSMTextTruncationTypeMiddle) {
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = TYSMTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = TYSMTextVerticalFormRotateAndMoveCharacterSet();
        ...
        void (^lineBlock)(YYTextLine *) = ^(YYTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = TYSMCTFontContainsColorBitmapGlyphs(font);
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[TYSMTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[TYSMTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[TYSMTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[TYSMTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[TYSMTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[TYSMTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[TYSMTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[TYSMTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[TYSMTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (TYSMTextAttachment *attachment in line.attachments) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [TYSMTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [TYSMTextUnarchiver unarchiveObjectWithData:textData];
...
- (BOOL)_insideEmoji:(YYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    ...
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!TYSMCTFontContainsColorBitmapGlyphs(font)) continue;
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!TYSMCTFontContainsColorBitmapGlyphs(font)) break;
...
- (YYTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    TYSMTextBinding *binding = [_text attribute:TYSMTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    ...
    if (line.range.length <= 2) {
        ...
        if (TYSMTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (TYSMTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (TYSMTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
...
- (YYTextRange *)textRangeByExtendingPosition:(YYTextPosition *)position {
    ...
    // binding range
    NSRange tRange;
    TYSMTextBinding *binding = [_text attribute:TYSMTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if (TYSMTextIsLinebreakChar(c0) && position.affinity == YYTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = TYSMTextLinebreakTailLength(str);
    ...
    if (verticalMove) { // up/down in text layout
        ...
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                ...
                NSUInteger lineBreakLen = TYSMTextLinebreakTailLength(subStr);
...
static void YYTextDrawRun(YYTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    ...
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(TYSMTextGlyphTransformAttributeName));
...
static void YYTextSetLinePatternInContext(TYSMTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == TYSMTextLineStylePatternCircleDot) {
...
static void YYTextDrawBorderRects(CGContextRef context, CGSize size, TYSMTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    TYSMTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = tysm_CGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        if ((border.lineStyle & 0xFF) == TYSMTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == TYSMTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void YYTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, TYSMTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = tysm_CGFloatPixelRound(position.y);
            y2 = tysm_CGFloatPixelRound(position.y + length);
            w = (styleBase == TYSMTextLineStyleThick ? lineWidth * 2 : lineWidth);
            
            CGFloat linePixel = tysm_CGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = tysm_CGFloatPixelHalf(position.x);
                } else {
                    x = tysm_CGFloatPixelFloor(position.x);
                }
            } else {
                x = position.x;
            }
            ...
            if (styleBase == TYSMTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == TYSMTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == TYSMTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = tysm_CGFloatPixelRound(position.x);
            x2 = tysm_CGFloatPixelRound(position.x + length);
            w = (styleBase == TYSMTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = tysm_CGFloatPixelHalf(position.y);
                } else {
                    y = tysm_CGFloatPixelFloor(position.y);
                }
            } else {
                y = position.y;
            }
...
static void YYTextDrawBlockBorder(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            TYSMTextBorder *border = attrs[TYSMTextBlockBorderAttributeName];
            if (!border) continue;
            ...
            do {
                ...
                if (next.row != lineContinueRow) {
                    TYSMTextBorder *nextBorder = [layout.text tysm_attribute:TYSMTextBlockBorderAttributeName atIndex:next.range.location];
...
static void YYTextDrawBorder(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, YYTextBorderType type, BOOL (^cancel)(void)) {
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == YYTextBorderTypeNormal ? TYSMTextBorderAttributeName : TYSMTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TYSMTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    TYSMTextBorder *iBorder = iAttrs[borderKey];
...
static void YYTextDrawDecoration(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, YYTextDecorationType type, BOOL (^cancel)(void)) {
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TYSMTextDecoration *underline = attrs[TYSMTextUnderlineAttributeName];
            TYSMTextDecoration *strikethrough = attrs[TYSMTextStrikethroughAttributeName];
            ...
            if (TYSMTextIsLinebreakString(runStr)) continue; // may need more checks...
            ...
            if (needDrawUnderline) {
                ...
                TYSMTextShadow *shadow = underline.shadow;
                while (shadow) {
                    if (!shadow.color) {
...
static void YYTextDrawAttachment(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    ...
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        TYSMTextAttachment *a = layout.attachments[i];
        ...
        rect = tysm_CGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = tysm_CGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                TYSMTextShadow *shadow = attrs[TYSMTextShadowAttributeName];
                TYSMTextShadow *nsShadow = [TYSMTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void YYTextDrawInnerShadow(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TYSMTextShadow *shadow = attrs[TYSMTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void YYTextDrawDebug(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, YYTextDebugOption *op) {
    ...
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / TYSMScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor) rect = tysm_CGRectPixelHalf(rect);
            else rect = tysm_CGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, tysm_CGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, tysm_CGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = tysm_CGFloatPixelHalf(line.position.x);
                CGFloat y1 = tysm_CGFloatPixelHalf(line.top);
                CGFloat y2 = tysm_CGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else {
                CGFloat x1 = tysm_CGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = tysm_CGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = tysm_CGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTLineNumberColor) {
            ...
            num.tysm_color = op.CTLineNumberColor;
            num.tysm_font = [UIFont systemFontOfSize:6];
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, tysm_CGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, tysm_CGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                if (op.CTRunNumberColor) {
                    ...
                    num.tysm_color = op.CTRunNumberColor;
                    num.tysm_font = [UIFont systemFontOfSize:6];
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, tysm_CGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, tysm_CGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)removeAttachmentFromViewAndLayer {
    YYAssertMainThread();
    for (TYSMTextAttachment *a in self.attachments) {
