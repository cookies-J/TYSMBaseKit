...
- (TYSMTextDirection)_keyboardDirection {
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return TYSMTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return TYSMTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return TYSMTextDirectionLeft;
    
    return TYSMTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(YYTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = tysm_RadiansToDegrees(rotation);
...
- (CGFloat)_updateMagnifier:(YYTextMagnifier *)mag {
    ...
    CGPoint captureCenter = [self tysm_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
    ...
    CGAffineTransform trans = TYSMCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = tysm_CGAffineTransformGetRotation(trans);
    ...
    if (mag.captureDisabled) {
        if (!mag.snapshot || mag.snapshot.size.width > 1) {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                CGRect rect = CGRectMake(0, 0, mag.tysm_width, mag.tysm_height);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, TYSMCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(YYTextMagnifier *)mag {
    ...
    CGPoint center = [self tysm_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
...
- (void)_updateSelectionGrabberDot:(YYSelectionGrabberDot *)dot selection:(YYTextSelectionView *)selection{
    ...
    if (selection.hostView.clipsToBounds == YES && dot.tysm_visibleAlpha > 0.1) {
        CGRect dotRect = [dot tysm_convertRect:dot.bounds toViewOrWindow:self];
        ...
        if (!dotInKeyboard) {
            ...
            if (tysm_CGRectGetArea(intersection) < tysm_CGRectGetArea(dotRect)) {
                CGFloat dist = tysm_CGPointGetDistanceToRect(tysm_CGRectGetCenter(dotRect), hostRect);
    ...
    CGPoint center = [dot tysm_convertPoint:CGPointMake(CGRectGetWidth(dot.frame) / 2, CGRectGetHeight(dot.frame) / 2) toViewOrWindow:self];
