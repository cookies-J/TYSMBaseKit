...
@implementation TYSMTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = [_font tysm_fontWithBold];
    _italicFont = [_font tysm_fontWithItalic];
    _boldItalicFont = [_font tysm_fontWithBoldItalic];
...
- (void)setColorWithBrightTheme {
    ...
    _border = [TYSMTextBorder new];
    _border.lineStyle = TYSMTextLineStyleSingle;
    ...
    _border.strokeWidth = tysm_CGFloatFromPixel(1);
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [text tysm_removeAttributesInRange:NSMakeRange(0, text.length)];
    text.tysm_font = _font;
    text.tysm_color = _textColor;
    ...
    [_regexHeader enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, whiteLen + sharpLen)];
        [text tysm_setColor:_headerTextColor range:NSMakeRange(r.location + whiteLen + sharpLen, r.length - whiteLen - sharpLen)];
        [text tysm_setFont:_headerFonts[sharpLen - 1] range:result.range];
    }];
    ...
    [_regexH1 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text tysm_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text tysm_setFont:_headerFonts[0] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text tysm_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexH2 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text tysm_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text tysm_setFont:_headerFonts[1] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text tysm_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexBreakline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        [text tysm_setColor:_controlTextColor range:result.range];
    }];
    ...
    [_regexEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, 1)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 1, 1)];
        [text tysm_setFont:_italicFont range:NSMakeRange(r.location + 1, r.length - 2)];
    }];
    ...
    [_regexStrong enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text tysm_setFont:_boldFont range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrongEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, 3)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 3, 3)];
        [text tysm_setFont:_boldItalicFont range:NSMakeRange(r.location + 3, r.length - 6)];
    }];
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text setTextUnderline:[TYSMTextDecoration decorationWithStyle:TYSMTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text setTextStrikethrough:[TYSMTextDecoration decorationWithStyle:TYSMTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexInlineCode enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location, len)];
        [text tysm_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - len, len)];
        [text tysm_setColor:_inlineTextColor range:NSMakeRange(r.location + len, r.length - 2 * len)];
        [text tysm_setFont:_monospaceFont range:r];
    ...
    [_regexLink enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text tysm_setColor:_linkTextColor range:r];
    }];
    ...
    [_regexLinkRefer enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text tysm_setColor:_controlTextColor range:r];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            [text tysm_setColor:_codeTextColor range:codeR];
            [text tysm_setFont:_monospaceFont range:codeR];
            TYSMTextBorder *border = [TYSMTextBorder new];
            border.lineStyle = TYSMTextLineStyleSingle;
            ...
            border.strokeWidth = tysm_CGFloatFromPixel(2);
            [text tysm_setTextBlockBorder:_border.copy range:codeR];
        }
    }];
...
@implementation TYSMTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        CTFontRef font = (__bridge CTFontRef)([text tysm_attribute:NSFontAttributeName atIndex:oneRange.location]);
        ...
        [atr setTextBackedString:[TYSMTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
        [text replaceCharactersInRange:oneRange withString:atr.string];
        [text tysm_removeDiscontinuousAttributesInRange:NSMakeRange(oneRange.location, atr.length)];
        [text addAttributes:atr.tysm_attributes range:NSMakeRange(oneRange.location, atr.length)];
