...
@implementation TYSMTextArchiver

+ (NSData *)archivedDataWithRootObject:(id)rootObject {
    if (!rootObject) return nil;
    NSMutableData *data = [NSMutableData data];
    TYSMTextArchiver *archiver = [[[self class] alloc] initForWritingWithMutableData:data];
...
@implementation TYSMTextUnarchiver

+ (id)unarchiveObjectWithData:(NSData *)data {
    if (data.length == 0) return nil;
    TYSMTextUnarchiver *unarchiver = [[self alloc] initForReadingWithData:data];
