...
/**
 ...
 @return YES or NO.
 */
static inline BOOL TYSMTextIsLinebreakChar(unichar c) {
    switch (c) {
        case 0x000D:
        case 0x2028:
        case 0x000A:
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL TYSMTextIsLinebreakString(NSString * _Nullable str) {
    if (str.length > 2 || str.length == 0) return NO;
    if (str.length == 1) {
        unichar c = [str characterAtIndex:0];
        return TYSMTextIsLinebreakChar(c);
    } else {
 ...
 */
static inline NSUInteger TYSMTextLinebreakTailLength(NSString * _Nullable str) {
    if (str.length >= 2) {
        unichar c2 = [str characterAtIndex:str.length - 1];
        if (TYSMTextIsLinebreakChar(c2)) {
        ...
        return TYSMTextIsLinebreakChar([str characterAtIndex:0]) ? 1 : 0;
    } else {
        return 0;
    }
}
 ...
 */
static inline NSTextCheckingType TYSMNSTextCheckingTypeFromUIDataDetectorType(UIDataDetectorTypes types) {
 ...
 */
static inline BOOL TYSMUIFontIsEmoji(UIFont *font) {
...
/**
 ...
 @return YES: the font is Emoji, NO: the font is not Emoji.
 */
static inline BOOL TYSMCTFontIsEmoji(CTFontRef font) {
    BOOL isEmoji = NO;
...
/**
 ...
 @return YES: the font is Emoji, NO: the font is not Emoji.
 */
static inline BOOL TYSMCGFontIsEmoji(CGFontRef font) {
    BOOL isEmoji = NO;
 ...
 */
static inline BOOL TYSMCTFontContainsColorBitmapGlyphs(CTFontRef font) {
 ...
 */
static inline BOOL TYSMCGGlyphIsBitmap(CTFontRef font, CGGlyph glyph) {
    if (!font && !glyph) return NO;
    if (!TYSMCTFontContainsColorBitmapGlyphs(font)) return NO;
 ...
 */
static inline CGFloat TYSMEmojiGetAscentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGFloat TYSMEmojiGetDescentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGRect TYSMEmojiGetGlyphBoundingRectWithFontSize(CGFloat fontSize) {
    CGRect rect;
    rect.origin.x = 0.75;
    rect.size.width = rect.size.height = TYSMEmojiGetAscentWithFontSize(fontSize);
    if (fontSize < 16) {
 ...
 */
static inline void TYSMUTF32CharToUTF16SurrogatePair(UTF32Char char32, UTF16Char char16[_Nonnull 2]) {
    char32 -= 0x10000;
 ...
 */
static inline UTF32Char TYSMUTF16SurrogatePairToUTF32Char(UTF16Char char16[_Nonnull 2]) {
 ...
 */
NSCharacterSet *TYSMTextVerticalFormRotateCharacterSet();

/**
 Get the character set which should rotate and move in vertical form.
 @return The shared character set.
 */
NSCharacterSet *TYSMTextVerticalFormRotateAndMoveCharacterSet();
