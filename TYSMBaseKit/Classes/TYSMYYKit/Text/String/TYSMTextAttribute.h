...
/// The attribute type
typedef NS_OPTIONS(NSInteger, TYSMTextAttributeType) {
    TYSMTextAttributeTypeNone     = 0,
    TYSMTextAttributeTypeUIKit    = 1 << 0, ///< UIKit attributes, such as UILabel/UITextField/drawInRect.
    TYSMTextAttributeTypeCoreText = 1 << 1, ///< CoreText attributes, used by CoreText.
    TYSMTextAttributeTypeYYText   = 1 << 2, ///< YYText attributes, used by YYText.
};

/// Get the attribute type from an attribute name.
extern TYSMTextAttributeType TYSMTextAttributeGetType(NSString *attributeName);

/**
 Line style in YYText (similar to NSUnderlineStyle).
 */
typedef NS_OPTIONS (NSInteger, TYSMTextLineStyle) {
    // basic style (bitmask:0xFF)
    TYSMTextLineStyleNone       = 0x00, ///< (        ) Do not draw a line (Default).
    TYSMTextLineStyleSingle     = 0x01, ///< (──────) Draw a single line.
    TYSMTextLineStyleThick      = 0x02, ///< (━━━━━━━) Draw a thick line.
    TYSMTextLineStyleDouble     = 0x09, ///< (══════) Draw a double line.
    
    // style pattern (bitmask:0xF00)
    TYSMTextLineStylePatternSolid      = 0x000, ///< (────────) Draw a solid line (Default).
    TYSMTextLineStylePatternDot        = 0x100, ///< (‑ ‑ ‑ ‑ ‑ ‑) Draw a line of dots.
    TYSMTextLineStylePatternDash       = 0x200, ///< (— — — —) Draw a line of dashes.
    TYSMTextLineStylePatternDashDot    = 0x300, ///< (— ‑ — ‑ — ‑) Draw a line of alternating dashes and dots.
    TYSMTextLineStylePatternDashDotDot = 0x400, ///< (— ‑ ‑ — ‑ ‑) Draw a line of alternating dashes and two dots.
    TYSMTextLineStylePatternCircleDot  = 0x900, ///< (••••••••••••) Draw a line of small circle dots.
};

/**
 Text vertical alignment.
 */
typedef NS_ENUM(NSInteger, TYSMTextVerticalAlignment) {
    TYSMTextVerticalAlignmentTop =    0, ///< Top alignment.
    TYSMTextVerticalAlignmentCenter = 1, ///< Center alignment.
    TYSMTextVerticalAlignmentBottom = 2, ///< Bottom alignment.
};

/**
 The direction define in YYText.
 */
typedef NS_OPTIONS(NSUInteger, TYSMTextDirection) {
    TYSMTextDirectionNone   = 0,
    TYSMTextDirectionTop    = 1 << 0,
    TYSMTextDirectionRight  = 1 << 1,
    TYSMTextDirectionBottom = 1 << 2,
    TYSMTextDirectionLeft   = 1 << 3,
};

/**
 The trunction type, tells the truncation engine which type of truncation is being requested.
 */
typedef NS_ENUM (NSUInteger, TYSMTextTruncationType) {
    /// No truncate.
    TYSMTextTruncationTypeNone   = 0,
    
    /// Truncate at the beginning of the line, leaving the end portion visible.
    TYSMTextTruncationTypeStart  = 1,
    
    /// Truncate at the end of the line, leaving the start portion visible.
    TYSMTextTruncationTypeEnd    = 2,
    
    /// Truncate in the middle of the line, leaving both the start and the end portions visible.
    TYSMTextTruncationTypeMiddle = 3,
};
...
UIKIT_EXTERN NSString *const TYSMTextBackedStringAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextBindingAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextShadowAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextInnerShadowAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextUnderlineAttributeName;

/// The value of this attribute is a `YYTextDecoration` object.
/// Use this attribute to add strikethrough (delete line) to a range of text.
/// The strikethrough will be drawn above text glyphs.
UIKIT_EXTERN NSString *const TYSMTextStrikethroughAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextBorderAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextBackgroundBorderAttributeName;

/// The value of this attribute is a `YYTextBorder` object.
/// Use this attribute to add a code block border to one or more line of text.
/// The border will be drawn below the text glyphs.
UIKIT_EXTERN NSString *const TYSMTextBlockBorderAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextAttachmentAttributeName;

/// The value of this attribute is a `TYSMTextHighlight` object.
/// Use this attribute to add a touchable highlight state to a range of text.
UIKIT_EXTERN NSString *const TYSMTextHighlightAttributeName;
...
UIKIT_EXTERN NSString *const TYSMTextGlyphTransformAttributeName;



#pragma mark - String Token Define

UIKIT_EXTERN NSString *const TYSMTextAttachmentToken; ///< Object replacement character (U+FFFC), used for text attachment.
UIKIT_EXTERN NSString *const TYSMTextTruncationToken; ///< Horizontal ellipsis (U+2026), used for text truncation  "…".
 ...
 */
typedef void(^TYSMTextAction)(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect);
...
/**
 ...
 string under the key named TYSMTextBackedStringAttributeName).
 
 It may used for copy/paste plain text from attributed string.
 Example: If :) is replace by a custom emoji (such as😊), the backed string can be set to @":)".
 */
@interface TYSMTextBackedString : NSObject <NSCoding, NSCopying>
...
/**
 ...
 the key named TYSMTextBindingAttributeName).
 ...
 selection and edit.
 */
@interface TYSMTextBinding : NSObject <NSCoding, NSCopying>
...
/**
 ...
 as the values for shadow attributes (stored in the attributed string under
 the key named TYSMTextShadowAttributeName or TYSMTextInnerShadowAttributeName).
 
 It's similar to `NSShadow`, but offers more options.
 */
@interface TYSMTextShadow : NSObject <NSCoding, NSCopying>
...
@property (nullable, nonatomic, strong) TYSMTextShadow *subShadow;  ///< a sub shadow which will be added above the parent shadow
...
/**
 ...
 the key named TYSMTextUnderlineAttributeName or TYSMTextStrikethroughAttributeName).
 ...
 */
@interface TYSMTextDecoration : NSObject <NSCoding, NSCopying>
+ (instancetype)decorationWithStyle:(TYSMTextLineStyle)style;
+ (instancetype)decorationWithStyle:(TYSMTextLineStyle)style width:(nullable NSNumber *)width color:(nullable UIColor *)color;
@property (nonatomic) TYSMTextLineStyle style;                   ///< line style
...
@property (nullable, nonatomic, strong) TYSMTextShadow *shadow;  ///< line shadow
@end
...
/**
 ...
 the key named TYSMTextBorderAttributeName or TYSMTextBackgroundBorderAttributeName).
 ...
 Example:
    ...
    ╰──────╯
 */
@interface TYSMTextBorder : NSObject <NSCoding, NSCopying>
+ (instancetype)borderWithLineStyle:(TYSMTextLineStyle)lineStyle lineWidth:(CGFloat)width strokeColor:(nullable UIColor *)color;
+ (instancetype)borderWithFillColor:(nullable UIColor *)color cornerRadius:(CGFloat)cornerRadius;
@property (nonatomic) TYSMTextLineStyle lineStyle;              ///< border line style
...
@property (nullable, nonatomic, strong) TYSMTextShadow *shadow; ///< border shadow
...
/**
 ...
 the key named TYSMTextAttachmentAttributeName).
 ...
 */
@interface TYSMTextAttachment : NSObject<NSCoding, NSCopying>
...
/**
 TYSMTextHighlight objects are used by the NSAttributedString class cluster
 as the values for touchable highlight attributes (stored in the attributed string
 under the key named TYSMTextHighlightAttributeName).
 ...
 highlighted state, the `attributes` in `TYSMTextHighlight` will be used to modify 
 (set or remove) the original attributes in the range for display.
 */
@interface TYSMTextHighlight : NSObject <NSCoding, NSCopying>
...
- (void)setShadow:(nullable TYSMTextShadow *)shadow;
- (void)setInnerShadow:(nullable TYSMTextShadow *)shadow;
- (void)setUnderline:(nullable TYSMTextDecoration *)underline;
- (void)setStrikethrough:(nullable TYSMTextDecoration *)strikethrough;
- (void)setBackgroundBorder:(nullable TYSMTextBorder *)border;
- (void)setBorder:(nullable TYSMTextBorder *)border;
- (void)setAttachment:(nullable TYSMTextAttachment *)attachment;
 ...
 */
@property (nullable, nonatomic, copy) TYSMTextAction tapAction;

/**
 Long press action when user long press the highlight, default is nil.
 If the value is nil, YYTextView or YYLabel will ask it's delegate to handle the long press action.
 */
@property (nullable, nonatomic, copy) TYSMTextAction longPressAction;
