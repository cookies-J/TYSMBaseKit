...
@interface TYSMTextView () <UIScrollViewDelegate, UIAlertViewDelegate, YYTextDebugTarget, YYTextKeyboardObserver> {
    
    YYTextRange *_selectedTextRange; /// nonnull
    YYTextRange *_markedTextRange;
    
    __weak id<TYSMTextViewDelegate> _outerDelegate;
    ...
    TYSMTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
...
- (void)_updateLayout {
    ...
    [text tysm_removeDiscontinuousAttributesInRange:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:TYSMTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:TYSMTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    if (_innerText.length == 0) {
        [text setTysm_attributes:_typingAttributesHolder.tysm_attributes]; // add for empty text caret
    }
    if (_selectedTextRange.end.offset == _innerText.length) {
        [_typingAttributesHolder.tysm_attributes enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
            [text setTysm_attribute:key value:value range:NSMakeRange(_innerText.length, 1)];
        }];
    }
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        ...
        container.truncationType = TYSMTextTruncationTypeEnd;
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        [newAttrs enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
            [hiText setTysm_attribute:key value:value range:_highlightRange];
        }];
...
- (void)_fixSelectionDot {
    ...
    CGPoint origin = [self tysm_convertPoint:CGPointZero toViewOrWindow:[YYTextEffectWindow sharedWindow]];
...
- (TYSMTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    TYSMTextHighlight *highlight = [text attribute:TYSMTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGPoint)_convertPointToLayout:(CGPoint)point {
    ...
    if (_innerLayout.container.isVerticalForm) {
        ...
        if (boundingSize.width < self.bounds.size.width) {
            if (_textVerticalAlignment == TYSMTextVerticalAlignmentCenter) {
                point.x += (self.bounds.size.width - boundingSize.width) * 0.5;
            } else if (_textVerticalAlignment == TYSMTextVerticalAlignmentBottom) {
...
- (void)_replaceRange:(YYTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    ...
    [_innerText tysm_removeDiscontinuousAttributesInRange:newRange];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        NSDictionary *attributes = [_innerText tysm_attributesAtIndex:index];
        if (!attributes) attributes = @{};
        _typingAttributesHolder.tysm_attributes = attributes;
        [_typingAttributesHolder tysm_removeDiscontinuousAttributesInRange:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:TYSMTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:TYSMTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (void)_updateOuterProperties {
    ...
    UIFont *font = _innerText.tysm_font;
    if (!font) font = _typingAttributesHolder.tysm_font;
    if (!font) font = [self _defaultFont];
    
    UIColor *color = _innerText.tysm_color;
    if (!color) color = _typingAttributesHolder.tysm_color;
    ...
    [self _setTypingAttributes:_typingAttributesHolder.tysm_attributes];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    TYSMTextHighlight *highlight = [TYSMTextHighlight highlightWithAttributes:_highlightTextAttributes];
                    [text tysm_setTextHighlight:highlight range:result.range];
                }
                if (_linkTextAttributes.count) {
                    [_linkTextAttributes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                        [text setTysm_attribute:key value:obj range:result.range];
                    }];
                }
            } break;
            default:
...
- (UIViewController *)_getRootViewController {
    ...
    if (!ctrl) ctrl = self.tysm_viewController;
    if (!ctrl) return nil;
...
- (void)_setTextParser:(id<TYSMTextParser>)textParser {
...
- (void)_initTextView {
    ...
    TYSMTextHighlight *highlight = [TYSMTextHighlight new];
    TYSMTextBorder * border = [TYSMTextBorder new];
...
- (void)setFont:(UIFont *)font {
    ...
    _typingAttributesHolder.tysm_font = font;
    _innerText.tysm_font = font;
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setTextColor:(UIColor *)textColor {
    ...
    _state.typingAttributesOnce = NO;
    _typingAttributesHolder.tysm_color = textColor;
    _innerText.tysm_color = textColor;
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setDataDetectorTypes:(UIDataDetectorTypes)dataDetectorTypes {
    ...
    NSTextCheckingType type = TYSMNSTextCheckingTypeFromUIDataDetectorType(dataDetectorTypes);
...
- (void)setTextParser:(id<TYSMTextParser>)textParser {
    if (_textParser == textParser || [_textParser isEqual:textParser]) return;
...
- (void)setTypingAttributes:(NSDictionary *)typingAttributes {
    ...
    [typingAttributes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [_typingAttributesHolder setTysm_attribute:key value:obj];
    }];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (_innerText.length > 0) {
        _typingAttributesHolder.tysm_attributes = [_innerText tysm_attributesAtIndex:_innerText.length - 1];
    }
...
- (void)setTextVerticalAlignment:(TYSMTextVerticalAlignment)textVerticalAlignment {
...
- (void)setPlaceholderText:(NSString *)placeholderText {
    if (_placeholderAttributedText.length > 0) {
        ...
        ((NSMutableAttributedString *)_placeholderAttributedText).tysm_font = _placeholderFont;
        ((NSMutableAttributedString *)_placeholderAttributedText).tysm_color = _placeholderTextColor;
    } else {
        if (placeholderText.length > 0) {
            ...
            atr.tysm_font = _placeholderFont;
            atr.tysm_color = _placeholderTextColor;
...
- (void)setPlaceholderAttributedText:(NSAttributedString *)placeholderAttributedText {
    ...
    _placeholderText = [_placeholderAttributedText plainTextForRange:NSMakeRange(0, _placeholderAttributedText.length)];
    _placeholderFont = _placeholderAttributedText.tysm_font;
    _placeholderTextColor = _placeholderAttributedText.tysm_color;
    [self _commitPlaceholderUpdate];
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            TYSMTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [TYSMImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img && p.PNGData) {
            img = [TYSMImage imageWithData:p.PNGData scale:kScreenScale];
        }
        if (!img && p.WEBPData) {
            img = [TYSMImage imageWithData:p.WEBPData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            if ([img conformsToProtocol:@protocol(TYSMAnimatedImage)]) {
                id<TYSMAnimatedImage> ani = (id)img;
                if (ani.animatedImageFrameCount > 1) {
                    TYSM_AnimatedImageView *aniView = [[TYSM_AnimatedImageView alloc] initWithImage:img];
                    if (aniView) {
                        content = aniView;
                    }
                }
            ...
            NSDictionary *attrs = _typingAttributesHolder.tysm_attributes;
...
- (id<TYSMTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<TYSMTextViewDelegate>)delegate {
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        TYSMTextBinding *binding = [_innerText attribute:TYSMTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange.asRange.length == 0) {
        ...
        if (needApplyHolderAttribute) {
            [_innerText setAttributes:_typingAttributesHolder.tysm_attributes range:_markedTextRange.asRange];
        }
        [_innerText tysm_removeDiscontinuousAttributesInRange:_markedTextRange.asRange];
    }
...
- (void)replaceRange:(YYTextRange *)range withText:(NSString *)text {
    ...
    if (_innerText.length > 0) {
        if (range.start.offset == 0 && range.end.offset == _innerText.length) {
            if (text.length == 0) {
                NSMutableDictionary *attrs = [_innerText tysm_attributesAtIndex:0].mutableCopy;
                [attrs removeObjectsForKeys:[NSMutableAttributedString tysm_allDiscontinuousAttributeKeys]];
                _typingAttributesHolder.tysm_attributes = attrs;
            }
        }
    } else { // no text
    ...
    if (useInnerAttributes) {
        [_innerText setTysm_attributes:_typingAttributesHolder.tysm_attributes];
    } else if (applyTypingAttributes) {
        NSRange newRange = NSMakeRange(range.asRange.location, text.length);
        [_typingAttributesHolder.tysm_attributes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            [_innerText setTysm_attribute:key value:obj range:newRange];
        }];
    }
    [self _parseText];
    [self _updateOuterProperties];
...
- (UITextWritingDirection)baseWritingDirectionForPosition:(YYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    ...
    NSDictionary *attrs = [_innerText tysm_attributesAtIndex:idx];
...
- (CGRect)caretRectForPosition:(YYTextPosition *)position {
    ...
    if (!CGRectIsNull(caretRect)) {
        ...
        return tysm_CGRectPixelRound(caretRect);
    }
    return CGRectZero;
}
...
- (NSDictionary *)textStylingAtPosition:(YYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
    if (_innerText.length == 0) return _typingAttributesHolder.tysm_attributes;
...
@interface TYSMTextView(IBInspectableProperties)
@end

@implementation TYSMTextView(IBInspectableProperties)
