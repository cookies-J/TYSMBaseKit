...
static dispatch_queue_t TYSMLabelGetReleaseQueue() {
...
@interface TYSMLabel() <YYTextDebugTarget, TYSMAsyncLayerDelegate> {
...
- (void)_updateLayout {
    ...
    _shrinkInnerLayout = [TYSMLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    dispatch_async(TYSMLabelGetReleaseQueue(), ^{
...
- (void)_trackDidLongPress {
    ...
    if (_highlight) {
        TYSMTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
...
- (TYSMTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    TYSMTextHighlight *highlight = [_innerText attribute:TYSMTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [TYSMLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case TYSMTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case TYSMTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case TYSMTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((TYSMAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _textVerticalAlignment = TYSMTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _textAlignment = NSTextAlignmentNatural;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [YYTextContainer new];
    _innerContainer.truncationType = TYSMTextTruncationTypeEnd;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            TYSMTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                TYSMTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    YYTextPosition *start = [YYTextPosition positionWithOffset:_highlightRange.location];
                    YYTextPosition *end = [YYTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:YYTextAffinityBackward];
                    YYTextRange *range = [YYTextRange rangeWithStart:start end:end];
...
- (void)setText:(NSString *)text {
    ...
    [_innerText tysm_removeDiscontinuousAttributesInRange:NSMakeRange(0, _innerText.length)];
    if (needAddAttributes) {
        _innerText.tysm_font = _font;
        _innerText.tysm_color = _textColor;
        _innerText.tysm_shadow = [self _shadowFromProperties];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = TYSMTextTruncationTypeNone;
            _innerText.lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = TYSMTextTruncationTypeStart;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = TYSMTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = TYSMTextTruncationTypeMiddle;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((TYSMAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (TYSMAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    TYSMTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    ...
    // create display task
    TYSMAsyncLayerDisplayTask *task = [TYSMAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        if (layoutNeedUpdate) {
            ...
            shrinkLayout = [TYSMLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == TYSMTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == TYSMTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
        ...
        point = tysm_CGPointPixelRound(point);
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (TYSMTextAttachment *a in drawLayout.attachments) {
        ...
        __strong TYSMLabel *view = (TYSMLabel *)layer.delegate;
        if (!view) return;
...
@interface TYSMLabel(IBInspectableProperties)
@end

@implementation TYSMLabel (IBInspectableProperties)
