...
TYSMSYNTH_DUMMY_CLASS(UIScreen_TYSMAdd);


@implementation UIScreen (TYSMAdd)

+ (CGFloat)tysm_screenScale {
...
- (CGRect)tysm_currentBounds {
    return [self tysm_boundsForOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
}

- (CGRect)tysm_boundsForOrientation:(UIInterfaceOrientation)orientation {
...
- (CGSize)tysm_sizeInPixel {
    CGSize size = CGSizeZero;
    
    if ([[UIScreen mainScreen] isEqual:self]) {
        NSString *model = [UIDevice currentDevice].tysm_machineModel;
