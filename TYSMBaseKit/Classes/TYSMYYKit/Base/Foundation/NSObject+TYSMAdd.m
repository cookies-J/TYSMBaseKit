...
TYSMSYNTH_DUMMY_CLASS(NSObject_TYSMAdd)


@implementation NSObject (TYSMAdd)
...
- (id)tysm_performSelectorWithArgs:(SEL)sel, ...{
    INIT_INV(sel, nil);
    [inv invoke];
    return [NSObject getReturnFromInv:inv withSig:sig];
}

- (void)tysm_performSelectorWithArgs:(SEL)sel afterDelay:(NSTimeInterval)delay, ...{
    INIT_INV(delay, );
    [inv retainArguments];
    [inv performSelector:@selector(invoke) withObject:nil afterDelay:delay];
}

- (id)tysm_performSelectorWithArgsOnMainThread:(SEL)sel waitUntilDone:(BOOL)wait, ...{
    INIT_INV(wait, nil);
...
- (id)tysm_performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thr waitUntilDone:(BOOL)wait, ...{
    INIT_INV(wait, nil);
    if (!wait) [inv retainArguments];
...
- (void)tysm_performSelectorWithArgsInBackground:(SEL)sel, ...{
    INIT_INV(sel, );
    [inv retainArguments];
...
#define case_size(_size_) \
    ...
    [inv setArgument:&arg atIndex:index]; \
            ...
            case_size(61) case_size(62) case_size(63) case_size(64)
                ...
                NSLog(@"TYSMYYKit performSelectorWithArgs unsupported type:%s (%lu bytes)",
...
- (void)tysm_performSelector:(SEL)sel afterDelay:(NSTimeInterval)delay {
...
- (void)tysm_setAssociateValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)tysm_setAssociateWeakValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_ASSIGN);
}

- (void)tysm_removeAssociatedValues {
    objc_removeAssociatedObjects(self);
}

- (id)tysm_getAssociatedValueForKey:(void *)key {
...
- (id)tysm_deepCopyWithArchiver:(Class)archiver unarchiver:(Class)unarchiver {
    id obj = nil;
    @try {
