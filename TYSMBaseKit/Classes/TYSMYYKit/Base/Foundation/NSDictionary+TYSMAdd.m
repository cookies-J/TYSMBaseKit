...
TYSMSYNTH_DUMMY_CLASS(NSDictionary_TYSMAdd)
...
#define TYSM_XMLText @"_text"
#define TYSM_XMLName @"_name"
#define TYSM_XMLPref @"_"

- (void)tysm_textEnd {
    _text = _text.tysm_stringByTrim.mutableCopy;
    if (_text.length) {
        NSMutableDictionary *top = _stack.lastObject;
        id existing = top[TYSM_XMLText];
        if ([existing isKindOfClass:[NSArray class]]) {
            [existing addObject:_text];
        } else if (existing) {
            top[TYSM_XMLText] = [@[existing, _text] mutableCopy];
        } else {
            top[TYSM_XMLText] = _text;
        }
    }
    _text = nil;
}
...
- (void)tysm_parser:(__unused NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName attributes:(NSDictionary *)attributeDict {
    ...
    if (!_root) node[TYSM_XMLName] = elementName;
...
- (void)tysm_parser:(__unused NSXMLParser *)parser didEndElement:(__unused NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName {
    ...
    [left removeObjectsForKeys:@[TYSM_XMLText, TYSM_XMLName]];
    for (NSString *key in left.allKeys) {
        [left removeObjectForKey:key];
        if ([key hasPrefix:TYSM_XMLPref]) {
            left[[key substringFromIndex:TYSM_XMLPref.length]] = top[key];
        }
    }
    if (left.count) return;
    
    NSMutableDictionary *children = top.mutableCopy;
    [children removeObjectsForKeys:@[TYSM_XMLText, TYSM_XMLName]];
    ...
    NSString *nodeName = top[TYSM_XMLName];
    if (!nodeName) {
