...
@implementation TYSMFileHash

+ (TYSMFileHash *)hashForFile:(NSString *)filePath types:(TYSMFileHashType)types {
    return [self hashForFile:filePath types:types usingBlock:nil];
}

+ (TYSMFileHash *)hashForFile:(NSString *)filePath types:(TYSMFileHashType)types usingBlock:(void(^)(UInt64 totalSize, UInt64 processedSize, BOOL *stop))block {
    TYSMFileHash *hash = nil;
