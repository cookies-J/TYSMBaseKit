...
+ (TYSMTimer *)timerWithTimeInterval:(NSTimeInterval)interval
                            target:(id)target
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"TYSMTimer init error" reason:@"Use the designated initializer to init." userInfo:nil];
...
- (instancetype)initWithFireTime:(NSTimeInterval)start
    ...
    dispatch_source_set_event_handler(_source, ^{[_self tysm_fire];});
    dispatch_resume(_source);
    return self;
}

- (void)tysm_invalidate {
...
- (NSTimeInterval)tysm_timeInterval {
