...
@implementation TYSMThreadSafeDictionary {
...
- (BOOL)isEqualToDictionary:(NSDictionary *)otherDictionary {
    ...
    if ([otherDictionary isKindOfClass:TYSMThreadSafeDictionary.class]) {
        TYSMThreadSafeDictionary *other = (id)otherDictionary;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:TYSMThreadSafeDictionary.class]) {
        TYSMThreadSafeDictionary *other = object;
        BOOL isEqual;
        dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
        dispatch_semaphore_wait(other->_lock, DISPATCH_TIME_FOREVER);
        isEqual = [_dic isEqual:other->_dic];
...
#pragma mark - custom methods for NSDictionary(TYSMAdd)

- (NSDictionary *)entriesForKeys:(NSArray *)keys {
    LOCK(NSDictionary * dic = [_dic tysm_entriesForKeys:keys]) return dic;
}

- (NSString *)jsonStringEncoded {
    LOCK(NSString * s = [_dic tysm_jsonStringEncoded]) return s;
}

- (NSString *)jsonPrettyStringEncoded {
    LOCK(NSString * s = [_dic tysm_jsonPrettyStringEncoded]) return s;
}

- (id)popObjectForKey:(id)aKey {
    LOCK(id o = [_dic tysm_popObjectForKey:aKey]) return o;
}

- (NSDictionary *)popEntriesForKeys:(NSArray *)keys {
    LOCK(NSDictionary * d = [_dic tysm_popEntriesForKeys:keys]) return d;
}
