...
/**
 TYSMMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 TYSMMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in TYSMMemoryCache is typically in constant time (O(1)).
 */
@interface TYSMMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(TYSMMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(TYSMMemoryCache *cache);
...
/**
 ...
 (such as tysm_removeObjectForKey:). Default is YES.
 */
 ...
 */
- (nullable id)tysm_objectForKey:(id)key;
 ...
 */
- (void)tysm_setObject:(nullable id)object forKey:(id)key;
...
/**
 ...
 objects that are put into it.
 */
- (void)tysm_setObject:(nullable id)object forKey:(id)key withCost:(NSUInteger)cost;
