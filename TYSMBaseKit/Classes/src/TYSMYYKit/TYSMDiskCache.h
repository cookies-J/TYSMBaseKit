...
/**
 TYSMDiskCache is a thread-safe cache that stores key-value pairs backed by SQLite
 and file system (similar to NSURLCache's disk cache).
 
 TYSMDiskCache has these features:
 ...
 */
@interface TYSMDiskCache : NSObject
...
/**
 ...
 @param block A block which will be invoked in background queue when finished.
 */
- (void)tysm_objectForKey:(NSString *)key withBlock:(void(^)(NSString *key, id<NSCoding> _Nullable object))block;
...
/**
 ...
 @param block  A block which will be invoked in background queue when finished.
 */
- (void)tysm_setObject:(nullable id<NSCoding>)object forKey:(NSString *)key withBlock:(void(^)(void))block;
