...
/**
 TYSMKVStorageItem is used by `TYSMKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface TYSMKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `TYSMKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use TYSMKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use TYSMKVStorageTypeFile to get better performance.
 * You can use TYSMKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, TYSMKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    TYSMKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    TYSMKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    TYSMKVStorageTypeMixed = 2,
};



/**
 TYSMKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for TYSMKVStorage is `initWithPath:type:`. 
 ...
 */
@interface TYSMKVStorage : NSObject

#pragma mark - tysm_attribute
///=============================================================================
/// @name tysm_attribute
///=============================================================================

@property (nonatomic, readonly) NSString *path;        ///< The path of this storage.
@property (nonatomic, readonly) TYSMKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(TYSMKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is TYSMKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is TYSMKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is TYSMKVStorageTypeMixed, then the item.value will be saved to file 
...
/**
 ...
 TYSMKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is TYSMKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is TYSMKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is TYSMKVStorageTypeMixed, then the `value` will be saved to file
 ...
 @return Whether succeed.
 */
- (BOOL)tysm_saveItemWithKey:(NSString *)key
                  value:(NSData *)value
...
/**
 ...
 @return Whether succeed.
 */
- (BOOL)tysm_removeItemForKeys:(NSArray<NSString *> *)keys;
 ...
 */
- (void)tysm_removeAllItemsWithProgressBlock:(nullable void(^)(int removedCount, int totalCount))progress
 ...
 */
- (nullable TYSMKVStorageItem *)tysm_getItemForKey:(NSString *)key;
 ...
 */
- (nullable TYSMKVStorageItem *)tysm_getItemInfoForKey:(NSString *)key;
 ...
 */
- (nullable NSData *)tysm_getItemValueForKey:(NSString *)key;

/**
 Get items with an array of keys.
 
 @param keys  An array of specified keys.
 @return An array of `TYSMKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TYSMKVStorageItem *> *)tysm_getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `TYSMKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TYSMKVStorageItem *> *)tysm_getItemInfoForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return A dictionary which key is 'key' and value is 'value', or nil if not 
    exists / error occurs.
 */
- (nullable NSDictionary<NSString *, NSData *> *)tysm_getItemValueForKeys:(NSArray<NSString *> *)keys;
