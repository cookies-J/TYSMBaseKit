...
@implementation TYSMThreadSafeArray {
...
- (BOOL)isEqualToArray:(NSArray *)otherArray {
    ...
    if ([otherArray isKindOfClass:TYSMThreadSafeArray.class]) {
        TYSMThreadSafeArray *other = (id)otherArray;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:TYSMThreadSafeArray.class]) {
        TYSMThreadSafeArray *other = object;
        BOOL isEqual;
        dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
        dispatch_semaphore_wait(other->_lock, DISPATCH_TIME_FOREVER);
...
#pragma mark - custom methods for NSArray(TYSMAdd)

- (id)randomObject {
    LOCK(id o = [_arr tysm_randomObject]) return o;
}

- (id)objectOrNilAtIndex:(NSUInteger)index {
    LOCK(id o = [_arr tysm_objectOrNilAtIndex:index]) return o;
}

- (void)removeFirstObject {
    LOCK([_arr tysm_removeFirstObject]);
}

- (id)popFirstObject {
    LOCK(id o = [_arr tysm_popFirstObject]) return o;
}

- (id)popLastObject {
    LOCK(id o = [_arr tysm_popLastObject]) return o;
}

- (void)appendObjects:(NSArray *)objects {
    LOCK([_arr tysm_appendObjects:objects]);
}

- (void)prependObjects:(NSArray *)objects {
    LOCK([_arr tysm_prependObjects:objects]);
}

- (void)insertObjects:(NSArray *)objects atIndex:(NSUInteger)index {
    LOCK([_arr tysm_insertObjects:objects atIndex:index]);
}

- (void)reverse {
    LOCK([_arr tysm_reverse]);
}

- (void)shuffle {
    LOCK([_arr tysm_shuffle]);
}
