...
static TYSMDispatchContext *TYSMDispatchContextCreate(const char *name,
                                                 uint32_t queueCount,
                                                 NSQualityOfService qos) {
    TYSMDispatchContext *context = calloc(1, sizeof(TYSMDispatchContext));
...
static void TYSMDispatchContextRelease(TYSMDispatchContext *context) {
    if (!context) return;
    if (context->queues) {
...
static dispatch_queue_t TYSMDispatchContextGetQueue(TYSMDispatchContext *context) {
...
static TYSMDispatchContext *TYSMDispatchContextGetForQOS(NSQualityOfService qos) {
    static TYSMDispatchContext *context[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = TYSMDispatchContextCreate("com.ibireme.TYSMkit.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = TYSMDispatchContextCreate("com.ibireme.TYSMkit.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = TYSMDispatchContextCreate("com.ibireme.TYSMkit.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = TYSMDispatchContextCreate("com.ibireme.TYSMkit.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = TYSMDispatchContextCreate("com.ibireme.TYSMkit.default", count, qos);
            });
            return context[4];
        } break;
    }
...
@implementation TYSMDispatchQueuePool {
    @public
    TYSMDispatchContext *_context;
}

- (void)dealloc {
    if (_context) {
        TYSMDispatchContextRelease(_context);
        _context = NULL;
    }
}

- (instancetype)initWithContext:(TYSMDispatchContext *)context {
    self = [super init];
...
- (instancetype)initWithName:(NSString *)name queueCount:(NSUInteger)queueCount qos:(NSQualityOfService)qos {
    ...
    self = [super init];
    _context = TYSMDispatchContextCreate(name.UTF8String, (uint32_t)queueCount, qos);
    if (!_context) return nil;
    _name = name;
    return self;
}

- (dispatch_queue_t)tysm_queue {
    return TYSMDispatchContextGetQueue(_context);
}

+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static TYSMDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[TYSMDispatchQueuePool alloc] initWithContext:TYSMDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[TYSMDispatchQueuePool alloc] initWithContext:TYSMDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
...
dispatch_queue_t TYSMDispatchQueueGetForQOS(NSQualityOfService qos) {
    return TYSMDispatchContextGetQueue(TYSMDispatchContextGetForQOS(qos));
}
